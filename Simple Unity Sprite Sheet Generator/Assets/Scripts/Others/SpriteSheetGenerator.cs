...
public static class SpriteSheetGenerator
{
    ...
    /// <returns></returns>
    public static Texture2D RenderToTexture(SpriteSheet sheet, List<SpriteNode> spriteNodes)
    {
        ...
        // Encode the texture into .png format and get it's byte data.
        var bytes = texture.EncodeToPNG();

        // Write the texture to a file.
        File.WriteAllBytes(Path.Combine(Serializer.SpriteSheetsOutputPath, $"{sheet.Name}.png"), bytes);
